...
class InvalidStateTransitionException implements Exception {
  ...
  InvalidStateTransitionException(this.entityId, this.fromState, this.toState)
...
class StateGuard {
  static final Map<MessageStatus, Set<MessageStatus>> _messageTransitions = {
    ...
    MessageStatus.sent: {
      MessageStatus.failed
    }, // Sometimes network failure post-send
    MessageStatus.failed: {
      MessageStatus.queued,
      MessageStatus.sending
    }, // Retries
  ...
  static final Map<PeerConnectionState, Set<PeerConnectionState>>
      _connectionTransitions = {
    PeerConnectionState.disconnected: {PeerConnectionState.connecting},
    PeerConnectionState.connecting: {
      PeerConnectionState.handshakePending,
  ...
  static void transitionMessage(
      String messageId, MessageStatus current, MessageStatus next,
      {String? correlationId}) {
    final allowed = _messageTransitions[current] ?? {};
    if (!allowed.contains(next) && current != next) {
      final e =
          InvalidStateTransitionException(messageId, current.name, next.name);
  ...
  static void transitionConnection(
      String transportId, PeerConnectionState current, PeerConnectionState next,
      {String? correlationId}) {
    final allowed = _connectionTransitions[current] ?? {};
    if (!allowed.contains(next) && current != next) {
      final e = InvalidStateTransitionException(
          transportId, current.storageValue, next.storageValue);
      EventSourcingLogger().logEvent(
        entityId: transportId,
        eventType: 'CONNECTION_TRANSITION_ERROR',
        payload: {
          'from': current.storageValue,
          'to': next.storageValue,
          'error': e.toString(),
        },
        correlationId: correlationId,
      );
